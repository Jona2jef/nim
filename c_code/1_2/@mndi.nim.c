...
N_LIB_PRIVATE N_NIMCALL(void, doWrite__ndi_12)(tyObject_NdiFile__9cEuXfAnHp9bnWFK409cMtsEw* f, tyObject_TSym__nnJKJFyjDGi5mnv8jwfTGQ* s, tyObject_ConfigRefcolonObjectType___LaK09ayL9alwKUad7rts0rTw* conf);
...
N_LIB_PRIVATE N_NIMCALL(void, reset__ndi_47)(tySequence__eHqUwlHGGaPw0TDPDeL79cQ** obj);
N_LIB_PRIVATE N_NIMCALL(void, reset__ndi_55)(NimStringDesc** obj);
...
N_LIB_PRIVATE N_NIMCALL(void, open__ndi_24)(tyObject_NdiFile__9cEuXfAnHp9bnWFK409cMtsEw* f, NimStringDesc* filename, tyObject_ConfigRefcolonObjectType___LaK09ayL9alwKUad7rts0rTw* conf) {
	NIM_BOOL T1_;
	T1_ = (NIM_BOOL)0;
	T1_ = isEmpty__options_4486(filename);
	(*f).enabled = !(T1_);
	{
...
N_LIB_PRIVATE N_NIMCALL(void, close__ndi_31)(tyObject_NdiFile__9cEuXfAnHp9bnWFK409cMtsEw* f, tyObject_ConfigRefcolonObjectType___LaK09ayL9alwKUad7rts0rTw* conf) {
	{
		if (!(*f).enabled) goto LA3_;
		(*f).f = open__systemZio_518((*f).filename, ((tyEnum_FileMode__ZJfK20XeZ9bv2j1pZjw9aswg) 1), ((NI) 8000));
		{
			NimStringDesc* T9_;
		...
		LA7_: ;
		{
			...
			L = T11_;
			{
				while (1) {
					if (!(i < L)) goto LA13;
					s = &(*f).syms->data[i];
					doWrite__ndi_12(f, (*s), conf);
					i += ((NI) 1);
				} LA13: ;
			}
		}
		close__systemZio_237((*f).f);
		reset__ndi_47((&(*f).syms));
		reset__ndi_55((&(*f).filename));
	}
	LA3_: ;
}
