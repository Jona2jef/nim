...
N_LIB_PRIVATE N_NIMCALL(void, handleShortOption__pureZparseopt_127)(tyObject_OptParser__l9cyoK9aLLTUH7LBRIbLqETg* p, NimStringDesc* cmd) {
	...
	}
	{
		...
		unsureAsgnRef((void**) (&(*p).val), substr__system_7821(cmd, i));
		(*p).pos = ((NI) 0);
		(*p).idx += ((NI) 1);
	}
	goto LA9_;
		...
		i += ((NI) 1);
		{
			...
			}
			{
				...
				LA57_: ;
				{
					...
					unsureAsgnRef((void**) (&(*p).val), substr__system_7821((*p).cmds->data[(*p).idx], i));
				}
				LA62_: ;
			}
			goto LA40_;
			LA44_: ;
			{
				...
				if (!(T66_)) goto LA68_;
				T69_ = (NIM_BOOL)0;
				T69_ = contains__stdZenumutils_43((*p).longNoVal->data, ((*p).longNoVal ? (*p).longNoVal->Sup.len : 0), (*p).key);
				T66_ = !(T69_);
				LA68_: ;
				T65_ = T66_;
				if (!(T65_)) goto LA70_;
...
N_LIB_PRIVATE N_NIMCALL(NimStringDesc*, npocmdLineRest)(tyObject_OptParser__l9cyoK9aLLTUH7LBRIbLqETg* p) {
	...
	result = NIM_NIL;
	T1_ = dotdot___stdZenumutils_80((*p).idx, ((NI) 1));
	T2_ = NIM_NIL;
	T2_ = X5BX5D___pureZparseopt_275((*p).cmds->data, ((*p).cmds ? (*p).cmds->Sup.len : 0), T1_);
	result = quoteShellCommand__pureZos_733(T2_->data, (T2_ ? T2_->Sup.len : 0));
	return result;
}
