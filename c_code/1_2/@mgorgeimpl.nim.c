...
N_LIB_PRIVATE N_NIMCALL(void, readOutput__gorgeimpl_9)(tyObject_ProcessObj__2rGNQnv9bU4hanI2g3svReg* p, tyTuple__XOVT24GHXwqiEj5GCsEGCA* Result);
...
N_LIB_PRIVATE N_NIMCALL(void, readOutput__gorgeimpl_9)(tyObject_ProcessObj__2rGNQnv9bU4hanI2g3svReg* p, tyTuple__XOVT24GHXwqiEj5GCsEGCA* Result) {
	...
	output = nospoutputStream(p);
	{
		while (1) {
			...
			NimStringDesc* T4_;
			T3_ = (NIM_BOOL)0;
			T3_ = atEnd__pureZstreams_63(output);
			if (!!(T3_)) goto LA2;
			T4_ = NIM_NIL;
			T4_ = readLine__pureZstreams_539(output);
...
appendString(T6_, cache);
		secureHash__stdZsha49_840(T6_->data, (T6_ ? T6_->Sup.len : 0), h);
		T7_ = NIM_NIL;
		T8_ = NIM_NIL;
		T8_ = dollar___stdZsha49_879(h);
		T7_ = rawNewString((T8_ ? T8_->Sup.len : 0) + 6);
appendString(T7_, ((NimStringDesc*) &TM__69a9b06YSkBJW7wqzuZfJ29aQ_3));
appendString(T7_, T8_);
		filename = toGeneratedFile__options_4226(conf, T7_, ((NimStringDesc*) &TM__69a9b06YSkBJW7wqzuZfJ29aQ_4));
		f = (FILE*)0;
		{
			NIM_BOOL T11_;
			...
			if (!(T11_)) goto LA12_;
			T11_ = open__systemZio_487(&f, filename, ((tyEnum_FileMode__ZJfK20XeZ9bv2j1pZjw9aswg) 0), ((NI) -1));
			LA12_: ;
			if (!T11_) goto LA13_;
			colontmp_ = readAll__systemZio_443(f);
			colontmp__2 = ((NI) 0);
		...
		p = nospstartProcess(cmd, workingDir, T15_, 0, ((tyObject_StringTableObj__V5PVrt9bIxZEeV7lfvqqtNg*) NIM_NIL), 12);
		{
			...
			write__pureZstreams_177(T20_, input);
			T21_ = NIM_NIL;
			T21_ = nospinputStream(p);
			close__pureZstreams_57(T21_);
		}
		LA18_: ;
		readOutput__gorgeimpl_9(p, Result);
		nospclose(p);
		readSuccessful = NIM_TRUE;
		{
			if (!((*Result).Field1 == ((NI) 0))) goto LA24_;
			writeFile__systemZio_567(filename, (*Result).Field0);
		}
		LA24_: ;
		popSafePoint();
		}
	...
	LA4_: ;
	{
		...
		p_2 = nospstartProcess(cmd, workingDir, T33_, 0, ((tyObject_StringTableObj__V5PVrt9bIxZEeV7lfvqqtNg*) NIM_NIL), 12);
		{
			...
			write__pureZstreams_177(T38_, input);
			T39_ = NIM_NIL;
			T39_ = nospinputStream(p_2);
			close__pureZstreams_57(T39_);
		}
		LA36_: ;
		readOutput__gorgeimpl_9(p_2, Result);
		nospclose(p_2);
		popSafePoint();
		}
		else {
