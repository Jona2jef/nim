...
N_LIB_PRIVATE N_NIMCALL(void, fillBuffer__pureZlexbase_16)(tyObject_BaseLexer__MAw8rJZtJ279bVV3TWMtXSg* L) {
	...
	toCopy = (NI)(((*L).buf ? (*L).buf->Sup.len : 0) - (NI)((*L).sentinel + ((NI) 1)));
	{
		...
		moveMem__system_1731(((void*) ((&(*L).buf->data[((NI) 0)]))), ((void*) ((&(*L).buf->data[(NI)((*L).sentinel + ((NI) 1))]))), ((NI) (toCopy)));
	}
	LA3_: ;
	T5_ = dotdot___pureZunicode_840(toCopy, (NI)((NI)((NI)(toCopy + (*L).sentinel) + ((NI) 1)) - ((NI) 1)));
	charsRead = readDataStr__pureZstreams_81((*L).input, (&(*L).buf), T5_);
		...
		s -= ((NI) 1);
		{
			while (1) {
				...
				LA19_: ;
				{
					...
					T22_ = dotdot___pureZunicode_840(oldBufLen, (NI)(((*L).buf ? (*L).buf->Sup.len : 0) - ((NI) 1)));
					charsRead = readDataStr__pureZstreams_81((*L).input, (&(*L).buf), T22_);
					{
