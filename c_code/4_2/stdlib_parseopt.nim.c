...
N_LIB_PRIVATE N_NIMCALL(void, initOptParser__pureZparseopt_51)(NimStringDesc* cmdline, tySet_tyChar__nmiMWKVIe46vacnhAFrQvw shortNoVal, tySequence__sM4lkSb7zS6F7OVMvW9cffQ* longNoVal, NIM_BOOL allowWhitespaceAfterColon, tyObject_OptParser__l9cyoK9aLLTUH7LBRIbLqETg* Result) {
	...
	LA3_: ;
	{
		NI T6_;
		T6_ = (NI)0;
		T6_ = paramCount__pureZos_1546();
		unsureAsgnRef((void**) (&(*Result).cmds), newSeq__systemZio_503(((NI) (T6_))));
		{
			NI i;
			NI colontmp_;
			NI res;
			i = (NI)0;
			colontmp_ = (NI)0;
			colontmp_ = paramCount__pureZos_1546();
			res = ((NI) 1);
			{
				while (1) {
					if (!(res <= colontmp_)) goto LA9;
					i = res;
					asgnRef((void**) (&(*Result).cmds->data[(NI)(i - ((NI) 1))]), paramStr__pureZos_1540(i));
					res += ((NI) 1);
				} LA9: ;
			}
		}
...
N_LIB_PRIVATE N_NIMCALL(void, handleShortOption__pureZparseopt_127)(tyObject_OptParser__l9cyoK9aLLTUH7LBRIbLqETg* p, NimStringDesc* cmd) {
	...
	}
	{
		...
		unsureAsgnRef((void**) (&(*p).val), substr__system_7766(cmd, i));
		(*p).pos = ((NI) 0);
		(*p).idx += ((NI) 1);
	}
	goto LA9_;
		...
		i += ((NI) 1);
		{
			...
			}
			{
				...
				LA57_: ;
				{
					...
					unsureAsgnRef((void**) (&(*p).val), substr__system_7766((*p).cmds->data[(*p).idx], i));
				}
				LA62_: ;
			}
			goto LA40_;
...
N_LIB_PRIVATE N_NIMCALL(NimStringDesc*, npocmdLineRest)(tyObject_OptParser__l9cyoK9aLLTUH7LBRIbLqETg* p) {
	...
	result = quoteShellCommand__pureZos_601(T2_->data, (T2_ ? T2_->Sup.len : 0));
	return result;
}
